...
export function FaviconGeneratorPage(): JSX.Element {
  ...
  const selectFile = (next: File): void => {
    ...
    void readImageDimensions(next).then((value) => { if (current === inspection.current) setDimensions(value); }).catch(() => undefined);
  };
  ...
  return <ToolPageTemplate tool={tool} meta={meta} breadcrumb={["Home", title]} workflow={{ state, error, onRetry: process, onReprocess: process }} children={{
    workspace: <><FileDropzone label={t("label.dropImage")} accept="image/png,image/jpeg,image/webp,image/avif" multiple={false} onFiles={(files) => selectFile(files[0])} compact={Boolean(file)} /><FileInfo files={file ? [file] : []} mode="single" onClear={() => { inspection.current += 1; clearResult(); setFile(null); setDimensions(null); }} compact={Boolean(file)} />{dimensions && dimensions.width !== dimensions.height ? <p role="status">{t("tool.favicon-generator.nonSquare")}</p> : null}</>,
    ...
    result: results.length ? <><ul className="batch-file-results">{results.map((result) => <li className="batch-file-results__item" key={result.fileName}><span className="batch-file-results__name">{result.fileName}</span><DownloadButton result={result} label={t("batch.downloadFile", { name: result.fileName })} /></li>)}</ul><DownloadCollectionButton results={results} fileName="favicon-set.zip" /></> : <p>{t("label.noResult")}</p>,
