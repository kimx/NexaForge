...
export function ImageResizePage(): JSX.Element {
  ...
  return <ToolPageTemplate tool={tool} meta={meta} breadcrumb={["Home", title]}
    workflow={{ state: processing, error, progress: files.length ? (completed / files.length) * 100 : 0, onRetry: process, onReprocess: process }} children={{
      workspace: <><FileDropzone label={t("label.dropImage")} accept="image/jpeg,image/png,image/webp" onFiles={selectFiles} onRejectedFiles={(rejections) => rejectFiles(rejections[0]?.message)} multiple maxSize={MAX_FILE_BYTES} /><FileInfo files={files} onClear={clearSelection} /></>,
      options: <div className="tool-form"><label>{t("label.width")}<input type="number" value={width} min={1} onChange={(event) => { setWidth(event.target.value); clearOutputs(); }} /></label><label>{t("label.height")}<input type="number" value={height} min={1} onChange={(event) => { setHeight(event.target.value); clearOutputs(); }} /></label><label className="checkbox"><input type="checkbox" checked={keepAspectRatio} onChange={(event) => { setKeepAspectRatio(event.target.checked); clearOutputs(); }} />{t("label.keepAspectRatio")}</label><label>{t("label.quality")}: {quality}<input type="range" min={1} max={100} value={quality} onChange={(event) => { setQuality(Number(event.target.value)); clearOutputs(); }} /></label><label>{t("label.format")}<select value={format} onChange={(event) => { setFormat(event.target.value as "jpeg" | "png" | "webp"); clearOutputs(); }}><option value="jpeg">JPG</option><option value="png">PNG</option><option value="webp">WebP</option></select></label><button className="btn primary" type="button" disabled={!files.length || !selectionValid || processing === "processing"} aria-busy={processing === "processing"} onClick={process}>{processing === "processing" ? t("button.processing") : t("button.process")}</button></div>,
