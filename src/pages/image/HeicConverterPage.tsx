...
export function HeicConverterPage(): JSX.Element {
  ...
  return <ToolPageTemplate tool={tool} meta={meta} breadcrumb={["Home", title]} workflow={{ state, error, onRetry: process, onReprocess: process }} children={{
    workspace: <><FileDropzone label={t("label.dropImage")} accept=".heic,.heif,image/heic,image/heif" onFiles={(next) => { setFiles(next.slice(0, 1)); clearResult(); }} multiple={false} /><FileInfo files={files} onClear={() => { setFiles([]); clearResult(); }} /></>,
