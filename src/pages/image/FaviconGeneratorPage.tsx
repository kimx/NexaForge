...
export function FaviconGeneratorPage(): JSX.Element {
  ...
  return <ToolPageTemplate tool={tool} meta={meta} breadcrumb={["Home", title]} workflow={{ state, error, onRetry: process, onReprocess: process }} children={{
    workspace: <><FileDropzone label={t("label.dropImage")} accept="image/png,image/jpeg,image/webp,image/avif" multiple={false} onFiles={(files) => selectFile(files[0])} /><FileInfo files={file ? [file] : []} onClear={() => { inspection.current += 1; clearResult(); setFile(null); setDimensions(null); }} />{dimensions && dimensions.width !== dimensions.height ? <p role="status">{t("tool.favicon-generator.nonSquare")}</p> : null}</>,
