...
export function JsonXmlPage(): JSX.Element {
  ...
  return (
    <ToolPageTemplate tool={tool} meta={meta} breadcrumb={["Home", title]} layout="split" showIdleResult
      workflow={{ state, error, onRetry: convert, onReprocess: convert }} children={{
        workspace: <div className="issue23-form code-workspace">
          <div className="code-workspace__controls code-workspace__controls--conversion">
            <fieldset className="code-workspace__direction"><legend>{t("tool.json-xml.title")}</legend>
            ...
            <button type="button" className="btn primary code-workspace__process" onClick={convert}>{t(outputIsXml ? "tool.json-xml.convertXml" : "tool.json-xml.convertJson")}</button>
          </div>
